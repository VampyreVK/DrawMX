...
public class ColorPicker : MonoBehaviour
{
    ...
    private void Update()
    {
        ...
        // Preview the color at the current sampling point if available.
        if (_lastHitPoint != null && _webcamTexture is not null && _webcamTexture.isPlaying)
        {
            Vector2 uv = WorldToTextureUV(_lastHitPoint.Value);
            Color previewColor = SampleAndCorrectColor(uv);

            // Update the line renderer's color for visualization.
            if (lineRenderer is not null)
            {
                lineRenderer.material.color = previewColor;
            }
        
            // Optionally, if you have a separate preview element, update its color too.
